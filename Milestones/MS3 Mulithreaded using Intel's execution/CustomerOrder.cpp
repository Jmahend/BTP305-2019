...
{
	auto it = std::find_if(std::execution::par, m_lstItem.begin(), m_lstItem.end(), [&item_name](const ItemInfo* item_info_obj) {return item_info_obj->m_itemName.compare(item_name) == 0 && item_info_obj->m_fillState == false; }); // 
	return (it != m_lstItem.end() ? (*it)->m_fillState : true);

}

bool CustomerOrder::MultipleItemsNeedFilling(std::string item_name) const
{
	int numOfItems = std::accumulate(m_lstItem.begin(), m_lstItem.end(), 0, [&item_name](int sum, const ItemInfo* item_info) {  
	
		if (item_info->m_itemName.compare(item_name) == 0 && item_info->m_fillState == false)
			 ++sum;
...
{
	auto it =std::find_if_not(std::execution::par, m_lstItem.begin(), m_lstItem.end(), [](const ItemInfo* item_info_obj) { return item_info_obj->m_fillState; });
...
void CustomerOrder::fillItem(Item& item, std::ostream& os)
{
	...
	auto it = find_if(std::execution::par, m_lstItem.begin(), m_lstItem.end(), [&item](const ItemInfo* item_info_obj) {return item.getName().compare(item_info_obj->m_itemName) == 0 && item_info_obj->m_fillState == false ; });
...
void CustomerOrder::display(std::ostream& os) const
{
	...
	std::for_each(m_lstItem.begin(), m_lstItem.end(), [&os](const ItemInfo* obj) {
		...
		os << "[";
		os <<  std::right << std::setfill('0') << std::setw(6) << obj->m_serialNumber << std::setfill(' ');
		os << "] " << std::left;
...
CustomerOrder& CustomerOrder::operator=(CustomerOrder&& cust_order) 
{
	...
	if (this != &cust_order) {
		...
		//Add parallel threading policy here

		std::move(std::execution::par, cust_order.m_lstItem.begin(), cust_order.m_lstItem.end(), m_lstItem.begin());
