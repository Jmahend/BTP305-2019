...
{
	auto it = std::find_if(m_lstItem.begin(), m_lstItem.end(), [&item_name](const ItemInfo* item_info_obj) {return item_info_obj->m_itemName.compare(item_name) == 0 && item_info_obj->m_fillState == false; }); // 
...
void CustomerOrder::fillItem(Item& item, std::ostream& os)
{
	...
	auto it = find_if(m_lstItem.begin(), m_lstItem.end(), [&item](const ItemInfo* item_info_obj) {return item.getName().compare(item_info_obj->m_itemName) == 0 && item_info_obj->m_fillState == false ; });
