...
LineManager::LineManager(const std::string& AssemblyLine_File_Name, std::vector<Task*>& AssemblyLine_Task, std::vector<CustomerOrder>& cust_orders)
{
	...
	if (file.good()) {
		...
		std::copy(AssemblyLine_Task.begin(), AssemblyLine_Task.end(), AssemblyLine.begin());

		ToBeFilled.resize(cust_orders.size());
		std::move(cust_orders.begin(), cust_orders.end(), ToBeFilled.begin());
		...
		while (std::getline(file, line)) {
			...
			while (cur_pos != std::string::npos) {
				...
				auto it1 = std::find_if(AssemblyLine.begin(), AssemblyLine.end(), [&tsk1](const Task* task) {return task->getName().compare(tsk1) == 0; });
				auto it2 = std::find_if(AssemblyLine.begin(), AssemblyLine.end(), [&tsk2](const Task* task) {return task->getName().compare(tsk2) == 0; });
